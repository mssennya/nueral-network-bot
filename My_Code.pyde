...
#Adds two lists item by item
def listAdd(n, l1, l2):
    for i in range(len(l1)):
        l1[i] = l1[i] + n*l2[i]
    return l1

#multiplies every item in a list by a scalar
def listScale(n,l1):
    for i in range(len(l1)):
        l1[i] *= n
    return l1
...
class AI(object):
    def __init__(self, l1, l2, l3):
        #Default values of Hidden layers and output layer. Overridden in the first frame
        ...
        self.w2 = range(l1 * l2)
        self.w3 = range(l2 * l3)
        #Other defaults which will mostly be overriden in the first frame or generation
        self.score = 0
    ...
    #Function that sets a specific layer of weights to an array 
    ...
    #Function that sets all three layers of weights at once
    def setWeights(self, w1, w2, w3):
        self.setWeight(1,w1)
        self.setWeight(2,w2)
        self.setWeight(3,w3)
        
    #Determines an output based off of game state inputs
    def output(self, inputs):
        self.calculate(inputs, self.w1, self.HL1)
        #maintains bias nueron
    ...
    #where the bulk of Nueral Network (NN) function takes place. 
    #takes in inputs of the characters in the game and moves the car accordingly
    ...
    #returns information about game state in the form of an array    
    def getStats(self):
    ...
    #judging NN performance based on goals scored/defended, distance to ball, and distance to goal
    ...
    #resets fitness score to 0. used at the end of each generation
    def resetFitness(self):
        self.score = 0
    
    #returns an array of values which correspond to the weights in a specific layer of the NN
    ...
    #performs certain updates. used if the NN is considered to have improved
    def improved(self):
        self.bestw1 = self.w1
        self.bestw2 = self.w2
        self.bestw3 = self.w3
    ...
    #sets the value of the reverted variable
    def setReverted(self, bool):
        self.justReverted = bool
    
    #returns the value of the reverted variable
    def getReverted(self):
        return self.justReverted
    
    #returns best score that the NN has achieved during training
    def getHighScore(self):
        return self.highscore
    
    #formats and prints best weights 
    ...
    #resets AI back to form which was determined to be most successful
...
#Start of physicsBall class (Parent class for cars and ball)
class physicsBall(object):
    #initializes physicsBall. sets location, velocity, radius, mass, and speed (acceleration)
    ...
    #shows a circle at the location of the physicsBall
    def show(self):
    ...
    #resets velocity and location to default values
    def reset(self,n):
        self.vel *= 0.0
        self.loc = PVector(W/2.0, H/2.0 + n*200)
    
    #determines effect of collision between physicsBall objects
    ...
    #returning values of location, velocity, radius, and mass
    def getLoc(self):
        return self.loc
    def getVel(self):
        return self.vel
    ...
    #set values for location and velocity
    def setLoc(self, x, y):
        self.loc.x = x
        self.loc.y = y
    def setVel(self,x,y):
...
#Start of Car class (child of physicsBall)
class Car(physicsBall):
    #initializing car (uses physicsBall initialization)
    def __init__(self,x,y,r,m,s,n):
        physicsBall.__init__(self,x,y,r,m,s)
        #set default values for number of goals, name, and movement control variables
        self.goals = 0
        self.name = n
        self.moveUp = False
        self.moveLeft = False
    ...
    #uses physicsBall show function
    #adds a line towards the mouse and writing the name and goals scored on the player
    def show(self):
    ...
    #makes it so that pressing a wasd button will set the ball moving in a direction...
    #...and releasing it will stop the movement, which fixed some early movement glitches
    ...
    def update(self):
        ...
        #friction, although in reality the car stops almost instantly
        self.vel.x*=0.9/log(abs(self.vel.x)+3)
        self.vel.y*=0.9/log(abs(self.vel.y)+3)
    
    #restricts car movement to withing the borders of the field
    def restrict(self):
    ...
    #adds a goal to goal count
    def score(self):
        self.goals+=1
    
    #resets location, velocity, and goals
...
#Start of ball class (child of physicsBall)
class Ball(physicsBall):
    #checks if the ball is inside a goal and awards goals accordingly, then resets
    #also restricts location withing the field, with exceptions that allows the ball to enter the goal
    def check(self):
        ...
        if(self.loc.x < xoffset+self.r/2):
            self.loc.x = xoffset+self.r/2
            self.vel.x *= -1
    #uses physicsBall update function, adds friction, and checks for restrictions/goals
    def update(self):
        physicsBall.update(self)
        self.vel *= (1-3/fps)
        self.check()

#creates cars and ball (notMyBall is the one that will be controlled by the AI)
...
#sets lengths of nueron layers
inputLength = 12+1
length1 = 8+1
length2 = 4+1
outputLength = 2

#creates AIs. carAI2 is the one that will control notMyCar. carAI1 will not be controlling myCar...
#...but will be used to calculate a fitness score for myCar
#also creates starting weights for the NNs
...
#weights that guarentee a win (against the AI which moves straight foreward)
...
#sets weights to guarentee wins (can be commented out to allow it to run and develope again)
carAI2.setWeights(winWeight1, winWeight2, winWeight3)

#change arrays are the step each weights array takes every generation
...
#some default values that influence AI learning
#var helps determine size of the step
#lastScore is defaulted to massively negative to guarentee that the AI improves in the first generation
#won is set to true once the AI-controlled car scores twice
var = 1.0
lastScore = -100000000
won = False
...
def draw():
    ...
    #AI2 controls notMyCar. myCar always moves in a straight line towards the ball
    ...
    #manages collisions between cars and ball
    ...
    #update and display each car and ball
    myCar.update()
    myCar.show()
    notMyCar.update()
    notMyCar.show()
    ...
    if(frameCount%(genLength*fps) == 0):
        ...
        #if AI failed last turn it will return to what had worked previously
        if(carAI2.getReverted()):
            carAI2.setReverted(False)
        else:        
            print var
            
            #judges AI progress from last generation
            improvement = carAI2.getFitnessScore() - lastScore
            lastScore = carAI2.getFitnessScore()
            improved = improvement > 0
            print improvement
            
            #determines if the AI won
            won = (notMyCar.getGoals() - myCar.getGoals() >= 2)
            
            if(improved):
                #if the AI improved, its weights will take another step in the same direction
                print "Improved"
                var = 1.0
                carAI2.improved()
            else:
                #if the AI didnt improve it will reset to what worked previously...
                #...and will take a step in a new direction
                carAI2.revertToBest()
                print "Reverted"
                #var will increase and decrease exponentially at the same time.
                #this is to ensure that if a small change is needed, it can...
                #make that step, and if a larger change is needed, it can also make that change
                var = 1/var
                if(var>=1):
                    var *= 1.1
                if(var<1):
                ...
                #picks new random step direction and scales step size by a variable
            ...
            #if the AI didnt win, it will take another step, if it won it will print what worked
            if(not won):
        ...
        #resets all locations, velocities, fitnessScores, etc. for next generation        
...
#if a key is pressed, it will print whatever the current best weights are
def keyPressed():
    carAI2.printBestWeights()
    #if(key == "r"):
        #carAI2.revertToBest()
